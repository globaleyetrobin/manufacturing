		...
		*/
        ...
        foreach ($input_data as $data) {
            ...
            if (isset($data['purchase_line_id'])) {
				...
				DB::table('purchase_lines')->where('id', $data['purchase_line_id'])->delete();
            ...
            /*    $purchase_line = PurchaseLine::findOrFail($data['purchase_line_id']);
				...
				$purchase_line->quantity = $new_quantity_f;
			...
			*/
			$insert=DB::table('purchase_lines')->insert([
						'transaction_id' => $transaction->id,
						'product_id' => $data['product_id'],
						'variation_id' => 0,
						'quantity' => $new_quantity_f,
						'pp_without_discount' => ($this->num_uf($data['pp_without_discount'], $currency_details)*$exchange_rate),
						'discount_percent' => $this->num_uf($data['discount_percent'], $currency_details),
						'purchase_price' => ($this->num_uf($data['purchase_price'], $currency_details)*$exchange_rate),
						'purchase_price_inc_tax' => ($this->num_uf($data['purchase_price_inc_tax'], $currency_details)*$exchange_rate),
						'item_tax' => ($this->num_uf($data['item_tax'], $currency_details)*$exchange_rate) 
						/*'quantity_received' => 0,
						'quantity_sold' => 0,
						'quantity_adjusted' => 0,
						'quantity_returned' => 0,
               ...
               /*$purchase_line = new PurchaseLine();
				$purchase_line->transaction_id=172;
				$purchase_line->product_id=7;
				$purchase_line->variation_id=8;
				$purchase_line->quantity=1;
				$purchase_line->pp_without_discount=60.00;
				$purchase_line->discount_percent=0.00;
				$purchase_line->purchase_price=60.00;
				$purchase_line->purchase_price_inc_tax=60.00;
				$purchase_line->item_tax=0.00;
				$purchase_line->tax_id=null;
				$purchase_line->quantity_received=0;
				$purchase_line->quantity_sold=0;
				$purchase_line->quantity_adjusted=0;
				$purchase_line->quantity_returned=0;
				$purchase_line->quantity_damaged=0;
				$purchase_line->mfg_date=null;
				$purchase_line->exp_date=null;
				$purchase_line->lot_number=null;
				$purchase_line->sub_unit_id=null;
				$purchase_line->created_at=null;
				$purchase_line->updated_at=null;
